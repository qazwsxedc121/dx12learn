...
SphereBuilder::MeshData SphereBuilder::BuildIcoSphere(float radius, uint32 subdivisionCount)
{
	MeshData meshData;

	subdivisionCount = std::min<uint32>(subdivisionCount, 6u);
	...
	for(uint32 i = 0; i < subdivisionCount; ++i)
	{
		Subdivide(meshData);
	}
	...
	for(uint32 i = 0; i < meshData.Vertices.size(); ++i)
	{
		...
		meshData.Vertices[i].TangentU = XMFLOAT3(-radius * sinf(phi) * sinf(theta), 0.0f, +radius * sinf(phi) * cosf(theta));
